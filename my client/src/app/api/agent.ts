...
axios.interceptors.response.use(async response => {
    ...
    switch (status) {
        case 404:
            ...
            break;
        case 500:
            toast.error("Enternal server error accurred");
...
const Baskets = {
    ...
    incrementItemQuantity: async (itemId: number, quantity: number = 1, dispatch: Dispatch) => {
        try {
            ...
            console.error("Failed to increment quantity in basket: ", error);
            throw error;
        }
    },
    decrementItemQuantity: async (itemId: number, quantity: number = 1, dispatch: Dispatch) => {
        try {
            ...
            console.error("Failed to decrement quantity in basket: ", error);
            throw error;
        }
    },
