...
import com.ecommerce.sportscenter.entity.OrderAggregate.ProductItemOrder;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Integer createOrder(OrderDto orderDto) {
        ...
        if (basketResponse == null) {
            log.info("Basket With ID {} not found", orderDto.getBasketId());
            return null;
        }
        // map basket items to order items
        List<OrderItem> orderItems = basketResponse.getItems().stream()
                .map(this::mapBasketOrderToOrderItem)
    ...
    private OrderItem mapBasketOrderToOrderItem(BasketItemResponse basketItemResponse) {
        if (basketItemResponse!= null){
            OrderItem orderItem = new OrderItem();
            orderItem.setItemOrder(mapBasketOrderToProduct(basketItemResponse));
    ...
    private ProductItemOrder mapBasketOrderToProduct(BasketItemResponse basketItemResponse) {
        ProductItemOrder productItemOrder = new ProductItemOrder();
        productItemOrder.setName(basketItemResponse.getName());
        productItemOrder.setPictureUrl(basketItemResponse.getPictureUrl());
        productItemOrder.setProductId(basketItemResponse.getId());
